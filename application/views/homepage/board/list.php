...
<!--컨트롤러에서 제공하는 크롤링 정보 있을 경우-->
<?php if(isset($crawling)): ?>
    <div id = "ad">
        <h2>
            ...
            <?php if($title == "교사 커뮤니티"): ?>
                피곤한 교사를 위한 맞춤 영양제 Top 10
            <?php elseif($title == "학부모 커뮤니티"): ?>
                우리 아이들이 좋아하는 장난감 Top 10
            <?php endif; ?>
        </h2>
        <p>네이버 실시간 검색어 순위에 기반한 자료입니다</p><hr>
        <div id= "crawling_slider">
            <?php $cnt = 1; ?>       
            <?php foreach($crawling as $row): ?>
                 <div>                   
                    <?=$row?>
                     <p class = "rank"><b><?=$cnt?>위</b></p>
                    <p class = "price">정가 | </p>
                </div>
                <?php $cnt++; ?>
            <?php endforeach; ?>       
