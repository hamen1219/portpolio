...
<!--
	컨트롤러에서 뷰로 전달되는 데이터 => $board, $side, $reply
	board : rs, num
	side  : rs (6 rows)
	reply : rs 
-->	
...
<section>
	...
	<div id = "left">
		<div id = "board">
			...
			<div id = "board_info">
				<div id = "img_cut">
					<img src="/img/users/<?=$board['user']?>/profile/<?=$board['user_img']?>">		
				</div>
				<ul class = "writer_info writer_info_hidden">
					<li><a href="/user/myroom/<?=$board['user']?>">유저상세보기</a></li>
					<li><a href="/user/my_board/<?=$board['user']?>">게시물보기</a></li>
				...
				<div id = "reply_slide">
					...
					********************* 댓글 출력 코드 ************************
					...
					<ul class = "writer_info writer_info_hidden reply_writer_info">
						...
						<li><a href="/user/my_board/<?=$row['user']?>">게시물보기</a></li>
						<li><a href="">쪽지보내기</a></li>
					</ul>
					...
					<div class = "reply_like">
						...
						<div>
							<?php if($row['contents'] !== ""): ?>
								<?php if($row['mygood'] == "1"): ?>
									<button class = "reply_btn_good" style = "background-color: white;" onclick="reply_ajax('good', this)">공감   <?=$row['good'] ?></button> |
								...
								<?php elseif($row['mypoor'] == "1"): ?>
									...
									<button class = "reply_btn_poor" style = "background-color: white;" onclick="reply_ajax('poor', this)"> 비공감 <?=$row['poor'] ?></button>
								<?php else: ?>
									<button class = "reply_btn_good" onclick="reply_ajax('good', this)">공감   <?=$row['good'] ?></button> |
									<button class = "reply_btn_poor" onclick="reply_ajax('poor', this)"> 비공감 <?=$row['poor'] ?></button>
								<?php endif; ?>
