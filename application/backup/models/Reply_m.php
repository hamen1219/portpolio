<?php 
	class Reply_m extends CI_Model{
		...
		public function getReply($board_num, $user = "")
		{
			...
			if($user !== "")
			{
				$select .= ',(select ifnull(good, 0) from like_r where user = u.id and reply_num = r.num) mygood,';
				$select .= '(select ifnull(poor, 0) from like_r where user = u.id and reply_num = r.num) mypoor,';
			}
